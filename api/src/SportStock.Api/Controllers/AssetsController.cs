...
public sealed class AssetsController(IAssetService service) : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        var rows = await service.ListCategoriesAsync(currentUser.ActiveClubId.Value, ct);
        return Ok(rows);
    }
        ...
        CancellationToken ct)
    {
        ...
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        var created = await service.CreateCategoryAsync(currentUser.ActiveClubId.Value, body, ct);
        ...
        CancellationToken ct)
    {
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        var asset = await service.GetAsync(id, currentUser.ActiveClubId.Value, ct);
        return Ok(asset);
    }
        ...
        CancellationToken ct)
    {
        ...
        var result = await service.UploadImageAsync(
            id, currentUser.ActiveClubId.Value, stream, image.ContentType, image.FileName, ct);
        return Ok(result);
    }
        ...
        CancellationToken ct)
    {
        ...
        var result = await service.AddBatchAsync(
            id, currentUser.ActiveClubId.Value, currentUser.UserId, body, ct);
        ...
        CancellationToken ct)
    {
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        var data = await service.GetDepreciationAsync(batchId, currentUser.ActiveClubId.Value, ct);
        return Ok(data);
    }
