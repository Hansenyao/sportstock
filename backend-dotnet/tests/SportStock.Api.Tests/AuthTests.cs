...
public sealed class AuthTests : IAsyncLifetime, IDisposable
{
    ...
    public AuthTests(DbFixture dbFixture)
    {
        ...
        _factory = new SportStockWebApplicationFactory().WithDb(dbFixture);
    }
    ...
    public async Task GetMe_Should_Return_401_When_Authorization_Header_Missing()
    {
        ...
        body.GetProperty("status_code").GetInt32().Should().Be(401);
    }
    ...
    [Fact]
    public async Task BCrypt_Should_Verify_A_Hash_Produced_By_Node_bcryptjs()
    {
        // $2a$10$... is the OpenBSD format both bcryptjs and BCrypt.Net-Next
        // emit. This hash was generated with `bcryptjs.hashSync('TestPass@123', 10)`
        // — verifying it via BCrypt.Net-Next proves the two libraries are
        // wire-compatible on existing user rows.
        const string nodeIssuedHash =
            "$2a$10$ifMb7XaLWqkXLuYK4Vsxu.0NXX0ej1Tn90sTtNkQzN7Bd7gOAjeOe";

        BCrypt.Net.BCrypt.Verify("TestPass@123", nodeIssuedHash).Should().BeTrue();
        BCrypt.Net.BCrypt.Verify("WrongPassword", nodeIssuedHash).Should().BeFalse();
    }
