...
export const createLoan: RequestHandler = async (req, res, next) => {
  try {
    const loan = await loanService.createLoan(req.user.club_id as string, req.user.id, req.user.name, req.body);
...
export const confirmReturn: RequestHandler = async (req, res, next) => {
  try {
    const loan = await loanService.confirmReturn(
      req.params.id, req.user.id, req.user.club_id as string, req.body.condition, req.body.notes
    );
    res.json(loan);
  } catch (err) {
    next(err);
