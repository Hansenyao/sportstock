...
public sealed class ReportsController(IReportService service) : ControllerBase
{
        ...
        [FromServices] ICurrentUser currentUser, CancellationToken ct)
    {
        ...
        return Ok(await service.GetSummaryAsync(currentUser.ActiveClubId.Value, ct));
    }
        ...
        [FromServices] ICurrentUser currentUser, CancellationToken ct)
    {
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        return Ok(await service.GetDepreciationAsync(currentUser.ActiveClubId.Value, ct));
    }
        ...
        [FromServices] ICurrentUser currentUser, CancellationToken ct)
    {
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        return Ok(await service.GetLoanUsageAsync(currentUser.ActiveClubId.Value, query, ct));
    }
        ...
        [FromServices] ICurrentUser currentUser, CancellationToken ct)
    {
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        return Ok(await service.GetRecentMovementsAsync(currentUser.ActiveClubId.Value, ct));
    }
        ...
        [FromServices] ICurrentUser currentUser, CancellationToken ct)
    {
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        return Ok(await service.GetMovementsAsync(currentUser.ActiveClubId.Value, query, ct));
    }
        ...
        [FromServices] ICurrentUser currentUser, CancellationToken ct)
    {
        if (currentUser.ActiveClubId is null)
            throw new AppException("You have not joined a club yet", 404);
        return Ok(await service.GetAlertsAsync(currentUser.ActiveClubId.Value, ct));
    }
}
